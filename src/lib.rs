        ...
        match self.status() {
            ...
            Status::Successful => {
                ...
                if &caller != &self.get_owner() {
                    return sc_error!("only owner can claim succesfull funnding");
                }
            ...
            Status::Failed => {
                ...
                if &deposit > &0 {
                    self.send_tx(&caller, &deposit, "reclaim failed fundin")
                }
                Ok(())
            }
        }
...
impl Status {
    ...
    fn from_u8(v: u8) -> Result<Status, DecodeError> {
        match v {
            0 => Result::Ok(Status::FundingPeriod),
            1 => Result::Ok(Status::Successful),
            2 => Result::Ok(Status::Failed),
